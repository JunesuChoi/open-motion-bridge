...
- `python -m open_motion_bridge sketch-image <photo> --output <project> --color-mode residual-pigment --closeup-mode phase-focus` creates an auditable HyperFrames drawing project. Prefer `residual-pigment`: a local semantic foreground matte makes the person the completion domain, suppresses background lettering from ink/detail selection, preserves the background as a blurred color field, and gives face strokes a thinner, lower-opacity treatment. Python then selects the next stored RGB stroke from remaining subject coverage/color error, builds mass → structure-aligned form → importance-weighted accent, then authors an actual-contour completion lock, deterministic settle, tool lift, delayed camera release, and final hold. The source image supplies analysis targets only; residual-mode HTML must not reveal, mask, or draw source pixels. `sampled-strokes` keeps the earlier Lab-region RGB traversal; `hybrid-paint` adds a subtle source-texture finish to that mode; `reveal` and legacy alias `paint` preserve the old mask behavior. `pen-follow` tracks the current tool closely; `phase-focus` holds slower regional close-ups; `none` keeps a full-frame view. `--closeup-zoom` accepts values above 1.0 through 3.0. For compatibility, specifying only a non-zero zoom selects `pen-follow`.
...
The sketch plan must retain ordered ink paths, every RGB/Lab pigment path, tool and camera tables, and audit metadata appropriate to the chosen mode. For `residual-pigment`, retain phase, importance, residual/coverage deltas, completion targets and measurements, final-lock identity, settle/tool-lift/camera-release timing, and the delayed full-frame hold. Selection is residual-driven and strokes follow local structure; do not substitute a fixed grid sweep, source-image reveal, or independently sampled circle stamps.
