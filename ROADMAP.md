...
Direction note (2026-08): the core video loop — analyze, EditSpec asset binding, deterministic generation, and render reprojection verification — is implemented. Still-image analysis, MotionSpec generation, and progressive ink-to-color drawing with residual-selected mass/form/accent RGB strokes, a contour completion lock, pigment settle, delayed camera release, measurable completion gates, and selectable close-up modes are also implemented. `residual-pigment` is the recommended source-free rendering path; the older Lab-region traversal, hybrid source-texture finish, and source-reveal mask remain compatibility choices. Video rendering itself is explicitly out of scope for this project: generated projects are handed to an external renderer, and this project verifies the externally rendered result. The phases below are re-prioritized around that boundary: verification depth first, then subject-aware still-image layers, expressive breadth, and skill packaging.
...
5. **Drawing verification.** Extend the deterministic residual-pigment fixtures with rendered-pixel monotonic painted-area, important-region color-error, maximum-hole, final-lock visibility, settle/tool-lift continuity, full-frame camera release, and brush/tool alignment measurements in addition to inspected snapshots.
