...
`residual-pigment` is the recommended coloring mode. Python precomputes a seek-safe plan by repeatedly selecting the largest remaining subject-matte coverage/color residual, weighted by structure and recorded face/hand importance heuristics. A local semantic foreground matte suppresses stage lettering and other background detail; the background is retained only as a blurred low-frequency color field, while face ink is deliberately thinner and lower-opacity. It stores real RGB and CIE Lab values, stroke importance, residual/coverage before and after every stroke, phase timings, coverage and hole measurements, and the final contour lock. Its generated HTML never draws or reveals source pixels: Canvas renders only the stored pigment paths and a deterministic settle pass. Camera release begins after settle and reaches full frame before the final hold.
